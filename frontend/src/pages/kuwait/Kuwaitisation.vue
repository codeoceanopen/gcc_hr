...
const profiles = createListResource({
	...
	fields: [
		...
		'business_size',
		'employee_count',
		'kuwaiti_employee_count',
		'non_kuwaiti_employee_count',
		'kuwaiti_percentage',
		'target_percentage',
		'gap',
		'compliance_status',
	...
	orderBy: 'kuwaiti_percentage asc',
	pageLength: 50,
	auto: true,
})
...
async function recalculate(company: string) {
	...
	try {
		await call('gcc_hr.api.kuwaitisation.recalculate_now', { company })
		toast.success(`Recalculated Kuwaitisation for ${company}.`)
		profiles.reload()
	} catch {
...
async function runSimulation() {
	...
	try {
		simResult.value = await call('gcc_hr.api.kuwaitisation.simulate', { ...simForm })
	} catch {
...
<template>
	<div>
		<PageHeader
			title="Kuwaitisation"
			description="Workforce-nationalization tracking for Kuwait. Target percentages are configuration (Kuwaitisation Requirement) -- never one universal number -- and the What-If Simulator never touches real employee data."
		/>

		<div class="space-y-6 p-6">
			<div class="rounded-xl border bg-white p-4">
				<p class="mb-3 text-sm font-medium text-gray-700">Company Kuwaitisation Status</p>
				<div v-if="!profiles.data?.length" class="text-sm text-gray-500">
					No Kuwaitisation Profiles yet -- these are created automatically when a company's country is set to
					Kuwait, and recalculated daily.
				</div>
				<table v-else class="w-full text-sm">
					<thead class="bg-gray-50 text-left text-xs uppercase text-gray-500">
						<tr>
							...
							<th class="px-2 py-1.5 font-medium">Kuwaiti</th>
							<th class="px-2 py-1.5 font-medium">Non-Kuwaiti</th>
							<th class="px-2 py-1.5 font-medium">Kuwaiti %</th>
					...
					<tbody class="divide-y">
						<tr v-for="p in profiles.data" :key="p.name">
							...
							<td class="px-2 py-1.5 text-gray-600">{{ p.kuwaiti_employee_count }}</td>
							<td class="px-2 py-1.5 text-gray-600">{{ p.non_kuwaiti_employee_count }}</td>
							<td class="px-2 py-1.5 text-gray-600">{{ p.kuwaiti_percentage }}%</td>
			...
			<div class="rounded-xl border bg-white p-4">
				...
				<div class="grid grid-cols-2 gap-3 sm:grid-cols-5">
					...
					<FormControl v-model="simForm.hire_kuwaiti" label="Hire Kuwaiti" type="number" />
					<FormControl v-model="simForm.hire_non_kuwaiti" label="Hire Non-Kuwaiti" type="number" />
					<FormControl v-model="simForm.terminate_kuwaiti" label="Terminate Kuwaiti" type="number" />
					<FormControl v-model="simForm.terminate_non_kuwaiti" label="Terminate Non-Kuwaiti" type="number" />
				</div>
				<div class="mt-4">
