...
const profiles = createListResource({
	...
	fields: [
		...
		'business_size',
		'employee_count',
		'emirati_employee_count',
		'non_emirati_employee_count',
		'emirati_percentage',
		'target_percentage',
		'gap',
		'compliance_status',
	...
	orderBy: 'emirati_percentage asc',
	pageLength: 50,
	auto: true,
})
...
async function recalculate(company: string) {
	...
	try {
		await call('gcc_hr.api.emiratisation.recalculate_now', { company })
		toast.success(`Recalculated Emiratisation for ${company}.`)
		profiles.reload()
	} catch {
...
async function runSimulation() {
	...
	try {
		simResult.value = await call('gcc_hr.api.emiratisation.simulate', { ...simForm })
	} catch {
...
<template>
	<div>
		<PageHeader
			title="Emiratisation"
			description="Workforce-nationalization tracking for the UAE. Target percentages are configuration (Emiratisation Requirement) -- never one universal number -- and the What-If Simulator never touches real employee data."
		/>

		<div class="space-y-6 p-6">
			<div class="rounded-xl border bg-white p-4">
				<p class="mb-3 text-sm font-medium text-gray-700">Company Emiratisation Status</p>
				<div v-if="!profiles.data?.length" class="text-sm text-gray-500">
					No Emiratisation Profiles yet -- these are created automatically when a company's country is set to
					United Arab Emirates, and recalculated daily.
				</div>
				<table v-else class="w-full text-sm">
					<thead class="bg-gray-50 text-left text-xs uppercase text-gray-500">
						<tr>
							...
							<th class="px-2 py-1.5 font-medium">Emirati</th>
							<th class="px-2 py-1.5 font-medium">Non-Emirati</th>
							<th class="px-2 py-1.5 font-medium">Emirati %</th>
					...
					<tbody class="divide-y">
						<tr v-for="p in profiles.data" :key="p.name">
							...
							<td class="px-2 py-1.5 text-gray-600">{{ p.emirati_employee_count }}</td>
							<td class="px-2 py-1.5 text-gray-600">{{ p.non_emirati_employee_count }}</td>
							<td class="px-2 py-1.5 text-gray-600">{{ p.emirati_percentage }}%</td>
			...
			<div class="rounded-xl border bg-white p-4">
				...
				<div class="grid grid-cols-2 gap-3 sm:grid-cols-5">
					...
					<FormControl v-model="simForm.hire_emirati" label="Hire Emirati" type="number" />
					<FormControl v-model="simForm.hire_non_emirati" label="Hire Non-Emirati" type="number" />
					<FormControl v-model="simForm.terminate_emirati" label="Terminate Emirati" type="number" />
					<FormControl v-model="simForm.terminate_non_emirati" label="Terminate Non-Emirati" type="number" />
				</div>
				<div class="mt-4">
