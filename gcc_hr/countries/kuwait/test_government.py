...
from gcc_hr.countries.kuwait.government import (
	generate_kuwaitisation_report,
	generate_wps_report,
	validate_kuwaitisation_report,
	validate_wps_report,
)
from gcc_hr.countries.kuwait.kuwaitisation import recalculate

TEST_COMPANY = "_Test GCC Kuwait Government Co"


class TestKuwaitGovernment(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_generate_kuwaitisation_report_reflects_kuwaitisation_profile(self):
		recalculate(TEST_COMPANY)
		result = generate_kuwaitisation_report(TEST_COMPANY)
	...
	def test_generate_kuwaitisation_report_fails_without_profile(self):
		with self.assertRaises(frappe.ValidationError):
			generate_kuwaitisation_report("_Test GCC Nonexistent Co")

	def test_validate_kuwaitisation_report_fails_without_profile(self):
		errors = validate_kuwaitisation_report("_Test GCC Nonexistent Co")
		self.assertTrue(errors)

	def test_validate_kuwaitisation_report_passes_after_recalculate(self):
		recalculate(TEST_COMPANY)
		errors = validate_kuwaitisation_report(TEST_COMPANY)
	...
	def test_generate_wps_report_lists_active_employees(self):
		employee = frappe.get_doc(
			{
				"doctype": "Employee",
				"first_name": "Test Kuwait Gov Employee",
