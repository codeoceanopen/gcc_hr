...
from gcc_hr.countries.kuwait.rules import (
	check_civil_id_expiry,
	check_contract_active,
	check_contract_salary_match,
	check_non_kuwaiti_has_civil_id,
	check_passport_expiry,
	check_work_permit_expiry,
	check_wps_registered,
)

TEST_COMPANY = "_Test GCC Kuwait Rules Co"


class TestKuwaitRules(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_kuwaiti_national_skips_civil_id_and_work_permit_checks(self):
		employee = self._make_employee("Test Kuwaiti National")
		frappe.db.set_value("Kuwait Employee Profile", employee.name, "nationality_status", "Kuwaiti")
		profile = frappe.get_doc("Employee Compliance Profile", employee.name)

		result, message, action = check_civil_id_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")

		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")

		result, message, action = check_non_kuwaiti_has_civil_id(employee.name, profile, None)
		self.assertEqual(result, "Passed")

	def test_civil_id_expiry_boundaries(self):
		employee = self._make_employee("Test Civil ID Boundaries")
		profile = frappe.get_doc("Employee Compliance Profile", employee.name)

		result, message, action = check_civil_id_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("No Civil ID expiry", message)

		frappe.db.set_value("Kuwait Employee Profile", employee.name, "civil_id_expiry", add_days(today(), -5))
		result, message, action = check_civil_id_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("expired", message)

		frappe.db.set_value("Kuwait Employee Profile", employee.name, "civil_id_expiry", add_days(today(), 200))
		result, message, action = check_civil_id_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")
	...
	def test_work_permit_expiry_boundaries(self):
		...
		frappe.db.set_value("Kuwait Employee Profile", employee.name, "work_permit_expiry", add_days(today(), -1))
		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("expired", message)

		frappe.db.set_value("Kuwait Employee Profile", employee.name, "work_permit_expiry", add_days(today(), 200))
		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")
	...
	def test_full_engine_run_includes_kuwait_rules(self):
		...
		for rule_code in (
			"KWT_CIVIL_ID_EXPIRY",
			"KWT_WORK_PERMIT_EXPIRY",
			"KWT_PASSPORT_EXPIRY",
			"KWT_NON_KUWAITI_CIVIL_ID_ON_FILE",
			"KWT_CONTRACT_ACTIVE",
			"KWT_CONTRACT_SALARY_MATCH",
			"KWT_WPS_REGISTERED",
		):
	...
	def test_wps_registered_fails_by_default_and_passes_when_registered(self):
		...
		frappe.db.set_value("Kuwait Employee Profile", employee.name, "wps_registered", 1)
		result, message, action = check_wps_registered(employee.name, profile, None)
		self.assertEqual(result, "Passed")
