...
"""Kuwait's HR Compliance Rule check_method implementations. Same fixed
...
identical in shape to the other countries' versions of the same checks --
see ARCHITECTURE.md's note on this duplication."""

import frappe
from frappe.utils import getdate, today


def _get_kuwait_profile(employee):
	if not frappe.db.exists("Kuwait Employee Profile", employee):
		return None
	return frappe.get_doc("Kuwait Employee Profile", employee)


def check_civil_id_expiry(employee, profile, rule):
	kp = _get_kuwait_profile(employee)
	if not kp:
		return ("Skipped", "No Kuwait Employee Profile found for this employee.", "")
	if kp.nationality_status != "Non-Kuwaiti":
		return ("Passed", "Kuwaiti national -- civil ID expiry not applicable.", "")
	if not kp.civil_id_expiry:
		return (
			"Failed",
			"No Civil ID expiry date on file.",
			"Record the Civil ID expiry date on the Kuwait Employee Profile.",
		)

	days_remaining = (getdate(kp.civil_id_expiry) - getdate(today())).days
	if days_remaining <= 0:
		return ("Failed", f"Civil ID expired {abs(days_remaining)} day(s) ago.", "Renew the employee's Civil ID immediately.")
	if days_remaining <= 30:
		return ("Failed", f"Civil ID expires in {days_remaining} day(s).", "Renew the employee's Civil ID before it expires.")
	return ("Passed", f"Civil ID valid for {days_remaining} more day(s).", "")
...
def check_work_permit_expiry(employee, profile, rule):
	...
	if not kp:
		return ("Skipped", "No Kuwait Employee Profile found for this employee.", "")
	if kp.nationality_status != "Non-Kuwaiti":
		return ("Passed", "Kuwaiti national -- work permit not applicable.", "")
	if not kp.work_permit_expiry:
		return (
			"Failed",
			"No work permit expiry date on file.",
			"Record the work permit expiry date on the Kuwait Employee Profile.",
		)

	days_remaining = (getdate(kp.work_permit_expiry) - getdate(today())).days
	if days_remaining <= 0:
		return (
			"Failed",
...
def check_non_kuwaiti_has_civil_id(employee, profile, rule):
	kp = _get_kuwait_profile(employee)
	if not kp:
		return ("Skipped", "No Kuwait Employee Profile found for this employee.", "")
	if kp.nationality_status != "Non-Kuwaiti":
		return ("Passed", "Kuwaiti national.", "")
	if not kp.civil_id_number:
		return (
			"Failed",
			"Non-Kuwaiti employee has no Civil ID number on file.",
			"Record the employee's Civil ID number on the Kuwait Employee Profile.",
		)
	return ("Passed", "Civil ID number on file.", "")
...
def check_wps_registered(employee, profile, rule):
	...
	if not kp:
		return ("Skipped", "No Kuwait Employee Profile found for this employee.", "")
	if not kp.wps_registered:
		return (
			"Failed",
			"Employee is not marked as registered with Kuwait's Wage Protection System (WPS).",
			"Register the employee's salary payment with WPS through the Public Authority for Manpower.",
		)
