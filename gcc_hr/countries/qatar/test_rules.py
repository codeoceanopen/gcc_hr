...
from gcc_hr.countries.qatar.rules import (
	check_contract_active,
	check_contract_salary_match,
	check_non_qatari_has_qid,
...
TEST_COMPANY = "_Test GCC Qatar Rules Co"


class TestQatarRules(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_qatari_national_skips_qid_and_work_permit_checks(self):
		employee = self._make_employee("Test Qatari National")
		frappe.db.set_value("Qatar Employee Profile", employee.name, "nationality_status", "Qatari")
		profile = frappe.get_doc("Employee Compliance Profile", employee.name)

		result, message, action = check_qid_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")

		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")

		result, message, action = check_non_qatari_has_qid(employee.name, profile, None)
		self.assertEqual(result, "Passed")

	def test_qid_expiry_boundaries(self):
		employee = self._make_employee("Test QID Boundaries")
		profile = frappe.get_doc("Employee Compliance Profile", employee.name)

		result, message, action = check_qid_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("No QID expiry", message)

		frappe.db.set_value("Qatar Employee Profile", employee.name, "qid_expiry", add_days(today(), 10))
		result, message, action = check_qid_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertTrue(action)

		frappe.db.set_value("Qatar Employee Profile", employee.name, "qid_expiry", add_days(today(), -5))
		result, message, action = check_qid_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("expired", message)

		frappe.db.set_value("Qatar Employee Profile", employee.name, "qid_expiry", add_days(today(), 200))
		result, message, action = check_qid_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")
	...
	def test_work_permit_expiry_boundaries(self):
		...
		frappe.db.set_value("Qatar Employee Profile", employee.name, "work_permit_expiry", add_days(today(), -1))
		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("expired", message)

		frappe.db.set_value("Qatar Employee Profile", employee.name, "work_permit_expiry", add_days(today(), 200))
		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")
	...
	def test_full_engine_run_includes_qatar_rules(self):
		...
		for rule_code in (
			"QAT_QID_EXPIRY",
			"QAT_WORK_PERMIT_EXPIRY",
			"QAT_PASSPORT_EXPIRY",
			"QAT_NON_QATARI_QID_ON_FILE",
			"QAT_CONTRACT_ACTIVE",
			"QAT_CONTRACT_SALARY_MATCH",
			"QAT_WPS_REGISTERED",
		):
			self.assertIn(rule_code, rule_codes)
		# Fresh Non-Qatari hire with no QID/work-permit/passport/contract/WPS
		# data on file yet: QID, Work Permit, Non-Qatari-no-QID and
		# Contract-Active all fail Critical/Blocking; WPS-Registered fails
	...
	def test_wps_registered_fails_by_default_and_passes_when_registered(self):
		...
		frappe.db.set_value("Qatar Employee Profile", employee.name, "wps_registered", 1)
		result, message, action = check_wps_registered(employee.name, profile, None)
		self.assertEqual(result, "Passed")
