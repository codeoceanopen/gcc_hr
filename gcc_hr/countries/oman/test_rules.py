...
from gcc_hr.countries.oman.rules import (
	check_contract_active,
	check_contract_salary_match,
	check_non_omani_has_resident_card,
	check_pasi_registered,
	check_passport_expiry,
	check_resident_card_expiry,
	check_work_permit_expiry,
)

TEST_COMPANY = "_Test GCC Oman Rules Co"


class TestOmanRules(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_omani_national_skips_resident_card_and_work_permit_checks(self):
		employee = self._make_employee("Test Omani National")
		frappe.db.set_value("Oman Employee Profile", employee.name, "nationality_status", "Omani")
		profile = frappe.get_doc("Employee Compliance Profile", employee.name)

		result, message, action = check_resident_card_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")

		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")

		result, message, action = check_non_omani_has_resident_card(employee.name, profile, None)
		self.assertEqual(result, "Passed")

	def test_resident_card_expiry_boundaries(self):
		employee = self._make_employee("Test Resident Card Boundaries")
		profile = frappe.get_doc("Employee Compliance Profile", employee.name)

		result, message, action = check_resident_card_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("No resident card expiry", message)

		frappe.db.set_value("Oman Employee Profile", employee.name, "resident_card_expiry", add_days(today(), -5))
		result, message, action = check_resident_card_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("expired", message)

		frappe.db.set_value("Oman Employee Profile", employee.name, "resident_card_expiry", add_days(today(), 200))
		result, message, action = check_resident_card_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")
	...
	def test_work_permit_expiry_boundaries(self):
		...
		frappe.db.set_value("Oman Employee Profile", employee.name, "work_permit_expiry", add_days(today(), -1))
		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Failed")
		self.assertIn("expired", message)

		frappe.db.set_value("Oman Employee Profile", employee.name, "work_permit_expiry", add_days(today(), 200))
		result, message, action = check_work_permit_expiry(employee.name, profile, None)
		self.assertEqual(result, "Passed")
	...
	def test_full_engine_run_includes_oman_rules(self):
		...
		for rule_code in (
			"OM_RESIDENT_CARD_EXPIRY",
			"OM_WORK_PERMIT_EXPIRY",
			"OM_PASSPORT_EXPIRY",
			"OM_NON_OMANI_RESIDENT_CARD_ON_FILE",
			"OM_CONTRACT_ACTIVE",
			"OM_CONTRACT_SALARY_MATCH",
			"OM_PASI_REGISTERED",
		):
	...
	def test_pasi_registered_fails_by_default_and_passes_when_registered(self):
		employee = self._make_employee("Test PASI Registered Rule")
		profile = frappe.get_doc("Employee Compliance Profile", employee.name)

		result, message, action = check_pasi_registered(employee.name, profile, None)
		self.assertEqual(result, "Failed")

		frappe.db.set_value("Oman Employee Profile", employee.name, "pasi_registered", 1)
		result, message, action = check_pasi_registered(employee.name, profile, None)
		self.assertEqual(result, "Passed")
